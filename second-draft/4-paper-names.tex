...
Some prior work has has suggested a functional explanation of Greenberg's correlations in terms of Dependency Length Minimization \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}.
This is the idea that languages order words so as to minimize the linear distance between words linked in head--dependent relationships.
On the basis of large-scale corpus evidence \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}, it has been confirmed that languages reduce dependency length.
...
\paragraph{Results}
Now present the correlations and the results.
...
Dryer \cite{dryer1992greenbergian} presents three correlations that are only relevant to part of the world's languages and are not annotated reliably in UD.
...
Further collapsing correlations that cannot be formalized individually in UD, we otained eight formalized correlations from Dryer's 15 correlations.
...
Confirming previous theoretical arguments, we see that optimizing for dependency length correctly accounts for all word order correlations. 
...
However, dependency length is both heuristic and stipulative.
...
There are other models of processing cost that make different predictions.

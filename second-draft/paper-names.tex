...
We go back to early functional work.
...
In this work, we show computationally that efficiency optimization accounts for the Greenbergian word order correlations.
That is, we show that word orders in natural language 
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/communication.png}
...
\begin{figure}
	...
	\caption{Given a set of dependency structures, grammars define a set of sentences. For simplicity, we use English words.  }
	\label{fig:pareto-plane}
\end{figure}



We model the process where a speaker communicates an utterance to a listener (Figure~\ref{fig:comm}).

We model the meaning that the speaker communicates via the predicate-argument structure.

We represent these in the format of Dependency Grammar:
This linguistic formalism draws directed arcs between syntactically related words, annotated with syntactic relations.

By the principle of compositionality \cite{frege1892sinn}, the meaning of a sentence is a function of the meanings of the parts and how they are combined; the dependency tree (including the labels on the arcs) specifies how the meanings of words are combined.
...
This quantity describes how homogeneous the language is, i.e., it is larger if the distribution over sentences is concentrated on a smaller number of frequent sentences. %how many distinct sentences there are.
%It is larger 
...
Fortunately, we have exacly the kind of data we need

For the dependency structures, we take real tree structures from corpora of 51 languages.
These corpora represent a typologically and genetically diverse group of languages.

For each of these corpora, we construct grammars that optimize efficiency of the linearized sentences.
(The original word orders of the languages do not enter.)

...
Following (CITE), we used as a very weakly informative prior a Student's $t$ prior with 3 degrees of freedom, mean 0, and scale $\sigma=10$ (i.e., the PDF $p$ is $\frac{1}{\sigma} p_3(x/\sigma)$).
The covariance matrix is modeled using the LKJ prior, as described in (CITE brms Bayesian multilevel).
We used four chains, with 5000 samples each (2500 warmup).
The chains had converged ($\hat{R} < 1.01$).

To verify robustness to more informative priors, we conducted the analysis (REF) with a tighter weakly informative prior.
...
We obtained qualitatively equivalent results for analyses on the levels of individual relations when estimating the model using \texttt{lme4} (CITE) and obtaining $p$-values from the resulting fit: For all eight correlations, the sign of $\alpha$ was positive ($p < 0.05$).
...
noun    &    relative clause    &    acl   &  24    \\ \hline
	adjective & std. of comp. & \multirow{2}{*}{obl} & --\\
verb    &    PP    &    & 22   \\
 \hline
%comp. &
%\multirow{3}{*}{NP}&
...
\end{tabular}
	\end{center}
	\caption{Greenbergian Correlations according to Dryer \cite{dryer1992greenbergian}, with operationalizations with Universal Dependencies.
	For reference, we also provide the numbers of the closest corresponding universals stated in Greenberg's original study.

...
\textcolor{red}{TODO I'll clean this up}
...
These quatities are estimated using two neural models:
...
A \key{language model} calculates the word-by-word surprisal of an utterance: $-\log p_\psi(u) = - \sum_{t=1}^{|u|} \log p_\psi(u_t|u_{1\dots t-1})$.
...
We can therefore apply stochastic gradient descent:
...
\textcolor{red}{fix which one of psi, phi goes with pred, parse}
...
Here, we validate our grammars by comparing the dominant orders of six syntactic relations that are also annotated in the World Atlas of Linguistic Structures (WALS, \cite{haspelmath2005world}).

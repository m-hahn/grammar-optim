...
We were able formalize eleven out of Dryer's seventeen correlations in UD. Six of these could not be expressed individually in UD, and were collapsed into three coarse-grained correlations.


We excluded four correlations that are not annotated reliably in UD, and are only relevant to some of the world's languages: Question particles, plural words (i.e., independent plural markers), and articles.
...
Dryer \cite{dryer1992greenbergian} counts this as a correlation since he describes the distribution of subject order as an interaction between a weak correlation with object order, and a very strong dominance principle favoring SV/SO orderings.
We focus on the modeling of correlations, and such dominance principles are beyond the scope of our experiments.
We therefore excluded this correlation.
...
\begin{tabular}{|ll|l|l}
	\hline
	\multicolumn{2}{|c|}{Correlates with...}   &          \multirow{2}{*}{Operationalization}       \\ 
	verb & object &    \\ \hline \hline % \textsc{verb} $\xrightarrow{obj}$ \textsc{noun}
%adp. &
adposition    &    NP    &    lifted\_case   & G3, G4   \\ \hline
copula  verb  &    predicate    &    lifted\_cop   & --    \\\hline
`want'    &    VP    &    xcomp   & G15   \\\hline
tense/aspect auxiliary    &    VP    &    \multirow{2}{*}{aux}   & G16, G13  \\
negative auxiliary    &    VP    &     \\ \hline
complementizer    &    S    &   \multirow{2}{*}{lifted\_mark}  & --    \\
adverbial subordinator & S & \\ \hline
noun    &    genitive    &    nmod & G2, G23   \\ \hline
noun    &    relative clause    &    acl   &  G24    \\ \hline
...
96~\% of entries where WALS records a dominant word order agree with the WALS judgment.

$*$ indicates that WALS does not list a dominant order, ? indicates that WALS has no data, and $--$ indicates that the relation does not occur in the corpus.

...
\section{Study 2: Results for Individual Languages}
...
\begin{figure} 
	...
	\end{center}
	\caption{Order of the eight correlates across 51 languages, in the real grammars (left) and predicted by optimizing for efficiency, predictability, parseability (right). Dark blue: Verb patterner \emph{precedes} object patterner (English, Arabic, ...). Light blue: Verb patterner \emph{follows} object patterner (Japanese, Hindi , ...). White cells indicate that the relation is not annotated in the dataset for the given language. \textcolor{red}{say more about how these predictions were generated}}\label{fig:per-lang}
...
\section{Study 2: Regression for Predicted Correlations}
...
%To verify robustness to more informative priors, we conducted the analysis (REF) with a much tighter prior $\mathcal{N}(0,1)$.

%Note that an intercept $\alpha \approx 2.19$ in the logistic model corresponds to a correlation that holds in 90\% of cases; we thus selected the prior $\mathcal{N}(0,2)$.
%we obtained a qualitatively equivalent result (posterior probability $0.932$ of predicting all eight correlations).
% N(0,2): 0.9863
...
\begin{tabular}{|l|ll|l|l|}
	\hline
&	\multicolumn{2}{|c|}{Correlates with...}   &          \multirow{2}{*}{UD Relation}   & \multirow{2}{*}{\cite{greenberg1963universals}}    \\ 
&	verb & object & &   \\ \hline \hline % \textsc{verb} $\xrightarrow{obj}$ \textsc{noun}
%adp. &
\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {1}}}&adposition    &    NP    &    case   & 3, 4   \\ \hline
\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {2}}}&copula  verb  &    predicate    &    cop   & --    \\\hline
\multirow{2}{*}{\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {3}}}}&tense/aspect auxiliary    &    VP    &    \multirow{2}{*}{aux}   & 16, 13  \\
&	negative auxiliary    &    VP    &    & -- \\ \hline
\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {4}}}&noun    &    genitive    &    nmod & 2, 23   \\ \hline
\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {5}}}&noun    &    relative clause    &    acl   &  24    \\ \hline
\multirow{2}{*}{\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {6}}}}&complementizer    &    S    &   \multirow{2}{*}{mark}  & --    \\
&	adverbial subordinator & S &  & -- \\ \hline
\multirow{2}{*}{\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {7}}}}&	adjective & std. of comp. & \multirow{2}{*}{obl} & --\\
&verb    &    PP    &    & 22   \\\hline
\raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {8}}}&`want'    &    VP    &    xcomp   & 15   \\\hline
 \hline
\end{tabular}
}
	\end{center}
	\caption{Greenbergian Correlations based on Dryer \cite{dryer1992greenbergian}, with operationalizations with Universal Dependencies.
...
\section{Study 1: Supporting Analyses for Study 1}
...
Following (CITE), we formalize the dependency length of a sentence as the sum of the distances of all pairs of syntactically related words, where distance is measured by the number of intervening words.
Prior work has suggested principles akin to DLM as approximating efficiency optimization of grammars (CITE).
...
\section{Study 2: Comparing Efficiency to its Components}\label{sec:posterior-number}
...
\section{Optimizing Grammars for Efficiency}

In this section, we describe the method we employ for creating grammars that are optimized for efficiency.
Our main methodological innovation is to carry out optimization in an extended space of grammars that interpolates continuously between different grammars (Section~\ref{sec:diff-gramm}).
...
More generally, if $\mathcal{L}_\theta$ is a differentiable ordering grammar (Section~\ref{sec:diff-gramm}), then $\mathcal{L}_\theta(t)$ is a \emph{probability distribution} over ordered sequences of words $\mathbf{w}$.
In the limit where $\mathcal{L}_\theta$ becomes deterministic, the distribution $\mathcal{L}_\theta(t)$ concentrates on a single ordering.
...
See Section (REF) for the stopping criterion.
...
Specifically, we constructed bigram models with Kneser-Ney smoothing (CITE).
...
\textcolor{red}{do same for language model, and for the other random grammars}
...
\begin{figure}
    ...
    \includegraphics[scale=.4]{figures/learning-parser-czech.pdf} 
        \caption{Parsing accuracy (measured in UAS) for optimized (light blue) and random (black) ordering grammar on Czech data, as a function of the fraction of total training data provided.\textcolor{red}{font size}}
...
Here, we validate our grammars by comparing the dominant orders of six syntactic relations that are also annotated in the World Atlas of Linguistic Structures (WALS, CITE).

...
\textcolor{red}{also provide p value, CI for how many baselines are better in either of the two dimensions}
...
In conclusion, $\lambda$ is constrained to be in $[0,1)$, which values closer to $1$ placing similar weights on both predictability and parsability, whereas values closer to $0$ diminish the role of predictability.
...
We also verified the robustness of our results by reporting results with $\lambda=1$ in Table REF>
...
In \cite{regier2007color, xu2014numeral, xu2016historical}, the cost function is the number of different forms; in \cite{kemp2012kinship} it is the complexity of the concepts encoded.
...
In addition to these models, which quantify the efficiency of communication systems, there is closely related work formalizing the optimal choice of specific utterances in context in models of pragmatic reasoning in reference games \cite{frank2012predicting,goodman2013knowledge, kao2014nonliteral}.
...
\section{Relation to DLM}

DLM turns out to be predicted by our model

\section{Other (Preregistered) Experiments}


Here we report the results of prior preregistered simulation runs.
...
\begin{tabular}{|c|ccc|c}
	\hline
	%&
& $\lambda=1.0$ & $\lambda=0.9$ & $\lambda=0.9$ \\ \hline \hline 
...
\end{tabular}
	\end{center}
\caption{Previous experiment with $\lambda=0.9$ (Preregistration)}\label{table:corr-resu}
\end{table}




\section{Languages and Corpora}

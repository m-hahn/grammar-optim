...
%Due to data availability, we restricted to 11 universals and further collapsed to the eight correlations shown in Table~\ref{table:corr-dryer}.
...
Figure~\ref{fig:pareto-per-lang}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../results/plane/pareto-plane-perLanguage.pdf}
\caption{per language results. Green: random baselines, Red: real grammar, blue: approximate Pareto frontier, computed from the optimized grammars.\textcolor{red}{would be good to have same colors as in main paper} All data are $z$-transformed.}\label{fig:pareto-per-lang}
\end{figure}

...
In Table~\ref{table:corr-resu} we report the results of our two previous, preregistered, simulations\footnote{\url{http://aspredicted.org/blind.php?x=8gp2bt}, \url{https://aspredicted.org/blind.php?x=bg35x7}. For the results of the locality simulations, see the DLM results in Table~\ref{tab:all-predictions-1}, see also Section~\ref{sec:DLM}.} together with results from the main experiment.
...
Giving similar weight to parseability and predictability -- that is, $\lambda$ close to $1$ -- results in more accurate predictions than choosing a small value of $\lambda$ such as $\lambda=0.0$. Note that $\lambda$ cannot take values smaller than zero, or greater than one, see Section \ref{sec:lambda}.
...
%We then compared parseability of random and optimized word order grammars on English data.
...
\textcolor{red}{phantom cite}\citep{diessel2001ordering, dryer1980positional, wals} 

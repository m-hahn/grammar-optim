...
Across theoretical paradigms, linguists have hypothesized that language is shaped by efficiency in computation~\cite{chomsky2005three,hauser2002faculty, berwick1984grammatical,hawkins1994performance} and communication~\cite{zipf1949human, Croft:Cruse:2004, Goldberg:2005, pinker1990natural}.
...
%In Arabic, the verb stands at the beginning, followed by the objecat and the adpositional phrase.
%In Arabic, `to' is expressed by a \emph{preposition}, so named because it precedes the word expressing `friend'. %The adpositional phrase starts with the adposition.
%In Japanese, it is expressed by a \textit{postposition}, so named because it follows the word denoting `friend'.
...
both basic statistical properties of languages~\cite{ferreri2003least},
...
%In information-theoretic formalizations of Zipf's ideas, the efficiency of a 
...
%This idea formalizes Zipf's forces of Unification and Diversification
Overall efficiency can be formalized as a weighted combination of terms representing the amount of information conveyed and the cost of communication:
...
This objective function has been proposed in prior computational tests of functional explanations~\cite{ferreri2003least,frank2012predicting,zaslavsky2018efficient,kemp2012kinship}.
...
With these choices, our objective function is equivalent to that proposed in \cite{ferrericancho2002zipf,ferrericancho2007global,futrell2017memory} as a general objective for communicative systems. %, taking dependency structures as the underlying meanings to be conveyed.
These works mainly differ in the cost function applied: can use the number of different utterances (CITE), the complexity of meanings \cite{kemp2012kinship}, and the entropy of possible utterances \cite{ferrericancho2002zipf,ferrericancho2007global,futrell2017memory}.



%CITE other syntactic work? LFG
%TODO the following sentences should go somewhere
%By the principle of compositionality \cite{frege1892sinn}, the meaning of a sentence is a function of the meanings of the parts and how they are combined.
%The dependency structure (including the labels on the arcs) specifies how the meanings of words are combined.
%Therefore, a listener needs to recover the information provided in the dependency structure in order to understand a sentence correctly.
...
TODO the following paragraphs depend on what visualization we choose
...
For the optimized languages, we indicate the posterior distribution of the proportion of satisfying languages, obtained from a mixed-effects logistic regression with language families as random effects (see \textit{Materials and Methods}).
Table~\ref{table:corr-dryer} shows that grammars optimized for efficiency accurately predict all eight correlations to hold at prevalences similar to actual languages.

We further evaluated predictions of efficiency optimization on the level of individual laguages (Figure~\ref{fig:per-lang}).
...
We conducted the same analysis for grammars optimized for predictability and parseability separately~(Figure~\ref{fig:posterior}).
%Predictability and parseability predict four and six correlations, respectively, making largely complementary predictions.
Efficiency optimization predicts all eight correlations to be universal with posterior probability $0.9911$. %The estimated posterior probability that efficiency optimization predicts less than all eight correlations is $0.0242$. The probability that it predicts less than seven of the correlations is $<10^{-4}$.
Predictability and parseability individually also predict six to eight correlations.



TODO better than this:

Finally, we compare to the predictions made by optimizing only for predictability, and only for parseability.

We further compared to the predictions made individually by optimizing for predictability and parseability.
We conducted a multivariate mixed-effects logistic regression model predicting which correlations an optimized grammar satisfies, entering the language and language family of the underlying corpus as random effects.\footnote{We coded language families according to \url{universaldependencies.org}.}
We estimated the posterior distribution of the number of correlations that the optimization process correctly predicts.
We conducted the same analysis for grammars optimized for predictability and parseability separately.
We used all eight runs of optimization to maximize statistical precision.
For every one of the correlations, we report the posterior estimated.
(These numbers control for random variation between language families, and are thus not directly comparable to data from the 51 languages).

...
This is illustrated in Figure~\ref{fig:corr-table}.
...
A commonly proposed functional explanation of Greenberg's correlations is in terms of Dependency Length Minimization \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}.
This is the idea that languages order words so as to minimize the linear distance between words linked in head--dependent relationships.
This has been confirmed on the basis of large-scale corpus evidence \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}, and has been justified based on the reasoning that long dependencies create high memory requirements for parsing and generation \cite{gibson1998linguistic,gibson2000dependency}.
...
In this work, we show that the word orders of languages have evolved to optimize efficiency, and that efficiency optimization accounts for the Greenbergian word order correlations.
...
Theoretical studies of word order typology have proposed Dependency Length Minimization as a unifying principle behind Greenberg's correlations \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}.
This is the idea that languages order words so as to minimize the linear distance between words linked in head--dependent relationships.
Our first contribution is to show that this principle indeed correctly predicts Greenberg's correlations.
We do this by creating counterfactual grammars that minimize dependency length of 



\section{Dependency Length Minimization Predicts Greenberg's Correlations}

Theoretical studies of word order typology have proposed Dependency Length Minimization as a unifying principle behind Greenberg's correlations \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}.
This is the idea that languages order words so as to minimize the linear distance between words linked in head--dependent relationships.

%(CITE) have made theoretical arguments that dependency length minimization should produce word order correlations.
%Here, we present the first large-scale test of this idea.

%It has been shown that languages reduce dependency length, compared to most randomly chosen orderings.
%However, the link to correlation universals is not trivial:
...
\end{dependency}
	...
	\caption{Dependency trees}\label{fig:dep-trees}
\end{figure}




The sentences in Figure~\ref{fig:dep-trees} are annotated with \key{dependency trees}:
these are directed trees describing the grammatical relations among words.
For example, the arcs labeled ``obj'' represent that the noun in question is the \emph{direct object} of the verb, rather than e.g. the subject or an indirect object.
A dependency arc is drawn from a \key{head} (e.g. \emph{visited} in Figure~\ref{fig:dep-trees}) to a \key{dependent} (e.g. \emph{friends}). Dependency trees can be defined in terms of many different syntactic theories \cite{corbett1993heads}. Although there are some differences in how different formalisms would draw trees for certain sentences, there is broad enough agreement about dependency trees that it has been possible to develop large-scale dependency-annotated corpora of text from dozens of languages \cite{nivre2017universal}.



The \key{dependency length} of a sentence ${\bf w}$ with dependency structure $t$ is the sum of the lengths of its dependency arcs, measured as the number of words spanned \cite{ferrericancho2004euclidean,liu2008dependency,gildea2010grammars,futrell2015largescale}.
%\begin{equation}
%    \label{eq:depl}
%\operatorname{DepL}(t; {\bf w}) = \sum_{i=1}^{\# {\bf w}} |i - head(t; w_i)|,
%\end{equation}
%where $head(t; i) \in \{1, ..., n\}$ is the index of the head of $w_i$. 
%On the basis of large-scale corpus evidence \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}, it has been confirmed that languages reduce dependency length, though a direct link to Greenger's correlations has not been established beyond theoretical arguments.
Large-scale corpus evidence across languages shows that natural languages order words in such a way as to reduce dependency length, compared to most random orderings \cite{futrell2015largescale,liu2017dependency,temperley2018minimizing}.
These data provide strong evidence that minimization of dependency length is itself a linguistic universal~\cite{temperley2018minimizing} 

While theoretical arguments suggest that DLM predicts Greenberg's correlations, this idea has not been tested on the basis of actual large-scale corpus data.
Prior theoretical work on dependency length minimization has argued that Greenberg's correlations minimize dependency length \cite{temperley2008dependency}; however, the link is not trivial:
Whether or not Greenberg's correlations theoretically minimize dependency length will depend on the distribution of tree topologies in actual language usage. %this depends on the topology of the dependency trees.


We test this idea by constructing languages that have minimal dependency length:
That is, we ask: Under the assumption that language has evolved to minimize dependency length, will Greenberg's correlations emerge?
We computationally construct hypothetical variants of real languages that are optimized to minimize dependency length.
If Greenberg's correlations emerge in these languages, then this provides evidence that DLM gives rise to these universals.

All natural languages have some degree of word order regularities that are specified in their grammar.
For instance, English places subjects before and objects after the verb (Figure~\ref{fig:dep-trees}).
Natural languages differ in the rules they apply: Some place the object after the verb, some place it before the verb.
This is illustrated in Figure~\ref{fig:dlm-grammars}: Grammars specify how dependency structures are encoded into strings of words.
...
We create optimized grammars that minimize the average dependency length.
Prior theoretical work on dependency length minimization has argued that certain general patterns minimize dependency length \cite{temperley2008dependency}, however, this depends on the topology of the dependency trees.
Answering these questions requires a sample of dependency structures as actually used by speakers of difefrent languages.
Such samples have recently become available with the Universal Dependencies (UD) project, which has collected and created dependency annotations for several dozens of different languages.
We use data from 51 languages.
These corpora represent a typologically and genetically diverse group of languages.
Previously, \cite{temperley2008dependency, CITE} have created grammars optimized for DLM; ours is the first study applying this to study correlation universals on a large-scale basis.


% (Frazier, 1985; Rijkhoff, 1990; Hawkins, 1994)
%Frazier, L. (1985). Syntactic complexity. In D. R. Dowty, L. Karttunen & A. M. Zwicky(Eds),Natural Language Parsing: Psychological, Computational, and TheoreticalPerspectives(pp. 129–189). Cambridge, UK: Cambridge University Press.
...
\begin{figure*}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/four-grammars-dlm.png}
	\caption{The DLM theory of word order universals: We show how four possible grammars can order the given dependency structure. In this example, Grammar 1 achieves the lowest overall dependency length. Indeed, this is the Given a set of dependency structures, grammars define a set of sentences. For simplicity, we use English words. Each grammar defines a consistent ordering for the different kinds of syntactic relations, e.g., Grammars 1 and 2 order the object after the verb, Grammars 3 and 4 order the object before the verb. \textbf{TODO This figure is just a draft sketch so far.}}
	\label{fig:dlm-grammars}
\end{figure*}
...
\begin{tabular}{|ll|l|l|l|ll|l|llllllll}
	...
	verb & object     & & & &  & \\ 
	\emph{visited} & \emph{friends} \\ \hline \hline % \textsc{verb} $\xrightarrow{obj}$ \textsc{noun}
...
%adp. &
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-lifted_case.pdf}   \textbf{81}$^{***}$    }
	...
	&    \multirow{2}{*}{\includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-lifted_case.pdf}  \textbf{76}$^{***}$ }   
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-efficiency-lifted_case.pdf} \textbf{68 \%}$^{***}$    } \\
...
copula    &    NP         
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-lifted_cop.pdf}   \textbf{81}$^{***}$    }
	...
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-lifted_cop.pdf}  \textbf{76}$^{***}$ }   
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-efficiency-lifted_cop.pdf} \textbf{61}$^{***}$  }  \\
...
auxiliary    &    VP       
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-aux.pdf}   \textbf{81}$^{***}$    }
	&    \multirow{2}{*}{\includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-langmod-aux.pdf}   }
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-aux.pdf}   \textbf{76}$^{***}$ }   
...
noun    &    genitive      
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-nmod.pdf} \textbf{81}$^{***}$    }
	&    \multirow{2}{*}{\includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-langmod-nmod.pdf}   }
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-nmod.pdf} \textbf{76}$^{***}$ }   
...
noun    &    relative clause      
	...
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-acl.pdf}\textbf{81}$^{***}$    }
	&    \multirow{2}{*}{\includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-langmod-acl.pdf}   }
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-acl.pdf} \textbf{76}$^{***}$ }   
...
complementizer    &    S        
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-lifted_mark.pdf} \textbf{81}$^{***}$    }
	...
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-lifted_mark.pdf}\textbf{76}$^{***}$ }   
...
verb    &    PP         
	...
	&   \multirow{2}{*}{ \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-depl-obl.pdf} \textbf{81}$^{***}$    }
	&    \multirow{2}{*}{\includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-langmod-obl.pdf}   }
	&   \multirow{2}{*}{  \includegraphics[width=0.03\textwidth]{../results/correlations/figures/correlations/correlation-parser-obl.pdf}\textbf{76}$^{***}$ }   
...
\paragraph{Results}
We show that optimizing grammars for DLM produces Greenberg's correlations.
%Dryer \cite{dryer1992greenbergian} provides an authoritative
We base our evaluation on the authoritative compilation of Greenberg's correlations by Dryer \cite{dryer1992greenbergian}, which documents 15 correlations, drawing on 625 languages. %, which we take as the basis of our evaluation.
...
%Considering those correlations that are annotated reliably in UD, we obtain eight correlations
We exclude three correlations that are only relevant to some of the world's languages, and collapse correlations that are not distinguished in UD.
...
%One further dependency is not annotated reliably in UD. %: the dependencies between question particles and verbs, those between nouns and plural words, and those between nouns and articles.
%Further collapsing correlations that cannot be formalized individually in UD, we otained eight formalized correlations (See SI for information on the other correlations). % from Dryer's 15 correlations.
We describe the resulting eight correlations in Table~\ref{table:corr-dryer}.
...
For each optimized grammar, we can check which of these correlations it satisfies.
We conducted a multivariate mixed-effects logistic regression model predicting, for each grammar, which of the eight correlations it satisfies.
We entered the language and language family of the underlying corpus as random effects, accounting for language-specific properties of tree topologies.\footnote{We coded language families according to \url{universaldependencies.org}.}
...
Optimizing for dependency length correctly accounts for all word order correlations. 



%\paragraph{Results}
%Results are shown in Table~\ref{table:results-dryer}.
%All eight correlations are confirmed in the models estimated from the real orderings. 
...
We have verified that dependency length minimization produces Greenberg's correlation universals.
However, dependency length is only a heuristic and stipulative measure of complexity: % heuristic and stipulative.
...
It is also stipulative: There is no a priori reason why dependency length should be the crucial determining factor determining word order.
While DLM has been justified based on the reasoning that long dependencies create high memory requirements for parsing and generation \cite{gibson1998linguistic,gibson2000dependency}, there are other models of processing cost that make different predictions.
In particular, surprisal is known to be a strong predictor of human processing effort, but it makes predictions different from and sometimes opposite to those made by dependency length.
DLM is also insufficient as an underlying principle of word order, as absolutely minimizing dependency length results in languages that have no consistent grammatical orders, making comprehension difficult.
Indeed, theoretical work has justified DLM as describing an aspect of a broader notion of language efficiency.
\cite{hawkins1994performance} theoetically argues that DLM enables more efficient parsing.
\cite{futrell2017noisycontext} argues that DLM arises from the optimization of predictability.
...
We show that the word order predictions of DLM arise from deeper principles of efficient communication.

Zipf \cite{zipf1949human} argued that language optimizes a tradeoff between speaker effort, the \emph{Force of Unification}, and listener effort, the \emph{Force of Diversification}.
...
Dependency structures provide a natural formalization of such structures (CITE).
%These are encoded in the dependency structure.
...
Speakers and listeners have shared knowledge of a \key{grammar} that specifies how dependency structures are encoded into sentences.
...
In Information Theory, the usefulness of a communication channel depends on (1) how costly encoding and transmission are, and (2) how precisely messages can be recovered from codes.
%Note that these terms correspond to Zipf's Forces of Unification Diversification
In keeping with Zipf's Forces of Unification and Diversification, transmission cost is lowered by channels that support fewer distinct codes, whereas recoverability is increased when the channel supports of larger range of distinct codess for different messages.
...
(1) the grammars of natural languages have evolved towards optimizing efficiency of communication, and
(2) this process of optimization accounts for Greenberg's order correlation universals.

%We use the same 51 corpora.
...
Again, the original orders of the actual languages do not enter this objective.
See SI for our method for creating optimized grammars.
...
We first demonstrate that real languages are relatively efficient compared to random baselines. For each of the 51 corpora, we generated ten baseline word order grammars.
These grammars have systematic word order regularities similar to natural language, but do not exhibit any correlations among the orderings of different syntactic relations (compare Figure~\ref{fig:dlm-grammars}).
...
Figure~\ref{fig:pareto-plane} also shows the average position of optimized languages. Languages appear to be attracted toward these points and away from the region of the baseline languages.
%We also see that several languages actually end up \emph{more} efficient than the computationally optimized languages.
...
%\subsection{Efficiency Explains Greenbergian Word Order Correlations}
\label{sec:greenberg}

We finally show that efficiency optimization predicts Greenberg's word order correlations. % in optimized grammars. 
Predictability and parseability predict four and six correlations, respectively, making complementary predictions.
Efficiency correctly predicts all the word order correlations.


%\subsection{Discussion}

%In Section~\ref{sec:relative-efficiency}, we found that word order grammars in the majority of UD languages have better efficiency than baseline word order grammars. Furthermore, in Section~\ref{sec:greenberg} we found that explicitly optimizing grammars for efficiency reproduces the Greenbergian correlations to the extent that they can be tested on the basis of available data.

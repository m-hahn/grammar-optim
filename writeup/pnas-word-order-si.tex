...
We consider two methods of quantifying the \key{degee of Pareto optimality}.
First, we quantify, for each value of $\lambda$ trading off parseability and predictability, how many baseline grammars are less efficient than the real language.
Second, we quantify how many baseline languages are more efficient across all values of $\lambda$.

Reecall the efficiency metric $R_\lambda := R_{Pars} + \lambda R_{Pred}$ with the tradeoff parameter $\lambda \in [0,1)$.
For each $\lambda$, we can quantify the fraction of baselines that have better efficiency than the real language
\begin{equation}
	O_\lambda := \frac{\# \{b \in B: R_{\lambda}(b) > R_{\lambda}(r)\}}{\# B}
\end{equation}
where $B$ is the set of baseline languages (make figure).
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{../results/plane/pareto-plane-perLanguage-isoCurve-smooth.pdf}
	\caption{Per-language results. Baseline distributions are estimated using Kernel Density estimation. Orange curves connect points estimated to be dominated by the same number of baselines as the real language, obtained by Bayesian inference using a Dirichlet Gaussian Process model.}\label{fig:isoline}
\end{figure}




We also considered a second method of quantifying the degree of Pareto optimality.
...
If we have access to the full distribution over baseline grammars, then the Pareto frontier is formed by exactly those grammars where $PO=0$ (make a figure).
In practice, we only have a limited sample of $50$ baseline grammar samples; so only an approximation of $PO$ is actually observable, and this approximation will be $0$ in a larger region of the plane (make a figure).

We aim to estimate $PO$, provide uncertainty intervals, and compare with the values of $PO$ attained by baseline languages, and estimate the isoline (cotour line)  of grammars having the same $PO$ value as the real language.

In order to estimate $PO$ from the 50 observed baseline samples, we did the following.
...
DPGMMs can be seen as a Bayesian version of Kernel Density Smoothing.
...
For each of these samples, we then computed $PO$ of the real language, and the isoline.
The collection of these results provides a set of samples from the posterior distribution over the true value of $PO$ and the true isoline.
We plot the posterior means of $PO$ and the isoline in Figure~\ref{fig:isoline}.
...
%We assumed that the distribution

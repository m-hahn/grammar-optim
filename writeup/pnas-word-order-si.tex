...
Efficiency combines predictability and parseability.
In the main paper (Study 1), we showed that languages tend to be optimized for these two quantities.
In this section, we show that languages are also optimized for the \emph{combination} of the two objectives.

We quantify the degree of optimality in two ways.
...
In Analysis 2, we quantify what fraction of baseline languages \emph{Pareto-dominates} the real language, showing that at most a very small fraction of grammars are more efficient than the real language.
...
Recall the efficiency metric $R_\lambda := R_{Pars} + \lambda R_{Pred}$ with the tradeoff parameter $\lambda \in [0,1)$.
...
TODO also visualize and explain

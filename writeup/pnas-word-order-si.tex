...
However, we also examined word order predictions for grammars optimized for only parseability or only predictability, in order to tease apart predictions made by these two components.
...
\textcolor{red}{have to improve DLM coverage}
...
We additionally report results when optimizing for Dependency Length Minimization (DLM), a heuristic approximation to efficiency that has been suggested in previous work. See Section~\ref{sec:DLM} for further discussion. \textcolor{red}{have to improve DLM coverage}   }
...
To prevent overfitting, we regularize each $a_\tau$, $b_\tau$ with a simple Bayesian prior $logit(a_\tau) \sim \mathcal{N}(0,1)$, $b_\tau \sim \mathcal{N}(0,1)$.\footnote{We implemented this regularized optimization as mean-field ELBO variational inference in Pyro~\cite{bingham2018pyro}.}
...
\textcolor{red}{TODO put language modeling and parsing formulas to make this more explicit}

...
Here we show that this is not a problem, since the ordering of the various relations subsumed under the UD label typologically strongly agree.

\begin{enumerate}
\item aux also covers modal, ... auxiliaries

Universals Archive 501 provides references, other people have made the statement for auxiliaires in general.

We also collected, for each language, all auxiliaires, that is, all dependents of the \emph{aux} dependency, occurring at least 10 times, and compared their dominant orders, which we operationalized as their more common order in the treebank (auxiliary-head or head-auxiliary).
In X languages, all extracted auxiliaries had the same dominant order.
In X languages, there were auxiliaires with different dominant orders.
		In Y cases, these were modal or passive auxiliaries, or uninflected elements.
		In Z cases, these were tense-aspect auxiliaries:
		In Persian and Afrikaans, not all tense-aspect auxiliaries 
		(1) In Persian, not all tense-aspect auxiliaries have the same order: E.g. budan as a perfect auxiliary follows the main verb (Mace p. 117), whereas xaastan xaah- as a future auxiliary precedes it (Mace p. 122).
		...
		(2) In Old Church Slavonic, most aux dependencies have the reflexive \emph{sebe} which tends to precede the head, whereas inflected verbal auxiliaries (primarily, forms of \emph{byti}) have opposite order, leading to a seeming violation of the correlation  \raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {3}}}.


Taken together, this shows that the dominant order of the \emph{aux} relation strongly coincides with that of inflected tense-aspect or negation auxiliaries.



\item nmod covers all noun-modifying NPs and PPs:
WALS evaluation, 37 entries, agreement in all but 2 cases (Danish, Swedish).

\item acl not  just relative clauses: In the WALS evaluation, acl agrees with the WALS entry for relative clauses: WALS has an entry in 36 languages where acl occurs in the treebank, which agrees with the extracted grammars in all but 3 languages (Estonian, Finnish, Tamil).
Also, UD provides a specific acl:relcl label in 21 of the languages.
In all but 3 languages, the majority order is the same for the general acl label as for the specific acl:relcl one (exceptions: Estonian, Finnish, Hindi).

\item obl covers adjunct NPs, ...: WALS evaluation: has 18 entries, no disagreement with extracted grammars

\item xcomp covers other control verbs, not just volition.

For each language, we extracted all lemmas of words heading an \emph{xcomp} dependency, occurring at least 2 times.
In X languages, all extracted words had the same dominant order.

In Ancient Greek, Latin, and Old Church Slavonic, verbs of volition agree with the extracted order, while several other verbs that do not indicate volition show opposite order.
In Gothic, the verb of volition (wiljan) has opposite order, resulting in an apparent violation of Correlation \raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {8}}}.

In four Germanic languages (Afrikaans, Danish, Dutch, and German), the verb of volition (Afrikaans \emph{wil}, Danish \emph{ville}, Dutch \emph{willen}, German \emph{wollen}) is mostly annotated with the \emph{aux} relation due to UD annotation guidelines, but in all languages, its dominant order (verb of volition before its complement) is compatible with the dominant order of the \emph{xcomp} dependency (head-initial).

Taken together, the order of the xcomp dependency tends to agree with that of most other xcomp dependencies, with the sole exception of Gothic.
\end{enumerate}
...
This efficiency metric~(\ref{eq:eff-general}-\ref{eq:efficiency-derived}) is equivalent to a deterministic version \cite{strouse2017deterministic} of the Information Bottleneck approach, which has been applied to modeling word meaning across different domains by \citet{zaslavsky2018efficient} and \citet{zaslavsky2019semantic}.
...
%\end{equation}
and it takes the maximally possible value zero if there is only a single utterance $\utterance$, in which case both $\operatorname{I}[\utterance; \tree]$ and $\operatorname{H}[\utterance]$ are zero.
...
Greenberg's original correlations/implications

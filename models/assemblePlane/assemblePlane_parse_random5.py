...
print("/u/scr/mhahn/deps/plane-parse-random5.tsv")
with open("/u/scr/mhahn/deps/plane-parse-random5.tsv", "w") as outFile:
  ...
  for BASE_DIR in dirs:
   ...
   if BASE_DIR in ["REAL_REAL", "RANDOM"]:
      ...
      for i in range(len(models)):
         ...
         #    print(parts)
             if len(parts) < 4 or parts[-3] not in ["RANDOM", "RANDOM5"] or parts[-2] != "model":
                continue
   ...
   if True:
     ...
     for language, model in models:
       existing = sorted([x for x in existingFiles if x.startswith("performance-"+language+"_soso") and "_"+model+".txt" in x])
       if len(existing) > 0:
          done += 1

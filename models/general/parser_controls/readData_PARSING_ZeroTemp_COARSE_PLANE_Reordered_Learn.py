...
FILE_NAME = "sososo_coarse_parser_reordered_learn"
...
#         shuffle(line["children_HD"])
   ...
   for r in range(len(linearized)):
      linearized[r]["grammar_position"] = r
   global mode
   shuffle(linearized)
   linearized = sorted(linearized, key=lambda x:x["posUni"])
...
rnn = nn.LSTM(2*pos_embedding_size, rnn_dim, lstm_layers, bidirectional=True, batch_first=True).cuda() # word_embedding_size + (2*
...
#         continue
       ...
       for i in range(maxLength+2):
          ...
          input_pos_pos.append(map(lambda x: 2 if i == 0 else (min(500, x[i-1]["grammar_position"])+3 if i <= len(x) else 0), batchOrdered))

...
FILENAME = __file__
with open("results/LOG"+language+"_"+FILENAME+"_model_"+str(myID)+".txt", "w") as outFile:
...
word_embeddings = torch.nn.Embedding(num_embeddings = vocab_size+3, embedding_dim = 70).cuda()
...
components = [word_embeddings,  rnn, decoder]
              ...
              for j in range(batchSize):
                 if input_words[i+1][j] != 0:
                    if input_words[i+1] > 2 and j == 0 and printHere:
                       print [itos[input_words[i+1][j]-3], itos_pos_ptb[input_pos_p[i+1][j]-3], lossesWord_tensor[i][j].data.cpu().numpy()]
                    wordNum += 1
           lossesExcludeMasked = lossesWord_tensor[:wordNum]
           reward = float((lossesExcludeMasked.pow(2).mean() - lossesExcludeMasked.mean().pow(2)).detach())

           if printHere:
                 print(lossesExcludeMasked)
                 print(reward)
                 print("@@@@@")
...
#       lossPOS = lossesPOS_tensor.sum()
 ...
 #      loss += lossPOS
       ...
       return loss, baselineLoss, 0, totalQuality, numberOfWords, lossWords.data.cpu().numpy(), 0, reward
...
def computeDevLoss():
   ...
   while True:
     ...
     for partition in partitions:
        ...
        _, _, _, newLoss, newWords, lossWords, lossPOS, reward = doForwardPass(current, train=False)
        devRewards += reward
        devLoss += newLoss
        devWords += newWords
        devLossWords += lossWords
        devLossPOS += lossPOS
   return devLoss/devWords, devLossWords/devWords, devLossPOS/devWords, devRewards/counterHere
...
while True:
  ...
  while True:
    ...
    for partition in partitions:
       ...
       loss, baselineLoss, policy_related_loss, _, wordNumInPass, lossWords, lossPOS, reward = doForwardPass(current)
       if wordNumInPass > 0:
  ...
  # Run on held-out set
  if True: 
          newDevLoss, newDevLossWords, newDevLossPOS, newDevReward = computeDevLoss()
          ...
          with open(save_path+"/language_modeling_coarse_plane_fixed/"+language+"_"+FILENAME+"_languageModel_performance_"+model+"_"+str(myID)+".tsv", "w") as outFile:
             ...
             print >> outFile, "\t".join(map(str, [newDevReward]))

...
parser.add_argument('--dropout_rate_parser', type=float, dest="dropout_rate_parser", default=0)
parser.add_argument('--beta1', type=float, dest="beta1", default=0.9)
parser.add_argument('--beta2', type=float, dest="beta2", default=0.9)
parser.add_argument('--clip_at_parser', type=float, dest="clip_at_parser", default=15)
parser.add_argument('--clip_norm_parser', type=int, dest="clip_norm_parser", default=2)
parser.add_argument('--pos_embedding_size_parser', type=int, dest="pos_embedding_size_parser", default=10)
parser.add_argument('--lstm_layers_parser', type=int, dest="lstm_layers_parser", default=1)
parser.add_argument('--shallow', type=bool, dest="shallow")
parser.add_argument('--rnn_dim_parser', type=int, dest="rnn_dim_parser", default=100)
parser.add_argument('--bilinearSize', type=int, dest="bilinearSize")
parser.add_argument('--input_dropoutRate_parser', type=float, dest="input_dropoutRate_parser", default=0)
parser.add_argument('--labelMLPDimension', type=int, dest="labelMLPDimension", default=100)
...
#   linearized.append({"word" : "EOS", "posUni" : "PUNCT", "posFine" : "PUNCT", "relevant_logprob_sum" : 0})
...
with open("results/LOG"+args.language+"_"+__file__+"_model_"+str(myID)+".txt", "w") as outFile:
...
word_embeddings_lm = torch.nn.Embedding(num_embeddings = vocab_size_lm+3, embedding_dim = 70).cuda()
...
components_lm = [word_embeddings_lm,  rnn_lm, decoder_lm,  baseline_lm, baseline_upos_lm]
...
#           pos_p_indices = Variable(torch.LongTensor(input_pos_p)).cuda()
           ...
           input_concat = words_layer
           inputEmbeddings = dropout_lm(input_concat) if train else input_concat

           output, hidden = rnn_lm(inputEmbeddings, None)
           baseline_predictions = log(vocab_size_lm) + log(len(posFine)) + baseline_lm(wordIndices) + baseline_upos_lm(pos_u_indices) 
          ...
          # word logits
           ...
           reward = (lossesWord_tensor.pow(2).mean() - lossesWord_tensor.mean().pow(2)).detach()

           baseline_shifted = baseline_predictions[1:].mean()

           baselineLoss_lm = torch.nn.functional.mse_loss(baseline_shifted, reward, size_average=False, reduce=False)
           ...
           for i in range(0,len(input_words)-1): 
              for j in range(args.batchSize):
                 if input_words[i+1][j] != 0:
                    policyGradientLoss_lm += (float(rewardMinusBaseline_lm) * batchOrdered[j][i]["relevant_logprob_sum"])
                    if input_words[i+1] > 2 and j == 0 and printHere:
                       print [itos[input_words[i+1][j]-3], lossesWord_tensor[i][j].data.cpu().numpy(), baseline_predictions[i+1][j].data.cpu().numpy()]
                    wordNum_lm += 1
...
def  doBackwardPass(loss_lm, baselineLoss_lm, policy_related_loss_lm):
       ...
       for param in parameters_lm_cached:
         ...
         if param.grad is None:
           assert False, (counterHere, param)
  ...
  while True:
    ...
    for partition in partitions:
       ...
       if counter % 50000 == 0:
          ...
          print "Saving"
          save_path = "results/"
          with open(save_path+"/"+args.language+"_"+__file__+"_model_"+str(myID)+".tsv", "w") as outFile:

...
FILENAME = "dotdotdot_COARSE_PLANE_noise_withoutPOS.py"
...
word_embeddings = torch.nn.Embedding(num_embeddings = vocab_size+3, embedding_dim = 200).cuda()



dropout = nn.Dropout(dropout_prob).cuda()

rnn = nn.LSTM(200, 128, 2).cuda()
...
def doForwardPass(current, train=True):
       ...
       if True:
           ...
           reward = (lossesWord_tensor).detach()
              ...
              for j in range(batchSize):
                 if input_words[i+1][j] != 0:
                    if input_words[i+1] > 2 and j == 0 and printHere:
                       print [itos[input_words[i+1][j]-3], lossesWord_tensor[i][j].data.cpu().numpy()]
                    wordNum += 1
       ...
       return loss, baselineLoss, 0, totalQuality, numberOfWords, lossWords.data.cpu().numpy(), 0
...
def computeDevLoss():
   ...
   while True:
     ...
     for partition in partitions:
        ...
        _, _, _, newLoss, newWords, lossWords, _ = doForwardPass(current, train=False)
        devLoss += newLoss
        devWords += newWords
        devLossWords += lossWords
   return devLoss/devWords, devLossWords/devWords, 0
...
while True:
  ...
  while True:
    ...
    for partition in partitions:
       ...
       loss, baselineLoss, policy_related_loss, _, wordNumInPass, lossWords, _ = doForwardPass(current)
       if wordNumInPass > 0:
  ...
  # Run on held-out set
  if True: 
          newDevLoss, newDevLossWords, _ = computeDevLoss()
          ...
          with open(save_path+"/language_modeling_coarse_plane_fixed_withoutPOS/"+language+"_"+FILENAME+"_languageModel_performance_"+model+"_"+str(myID)+".tsv", "w") as outFile:
             print >> outFile, language
             print >> outFile, "\t".join(map(str, devLosses))
             print >> outFile, "\t".join(map(str, devLossesWords))
             print >> outFile, "\t".join(map(str, [0 for x in devLossesWords]))

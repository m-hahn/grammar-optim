...
import os
#parsingDone = [x.split("_")[-1][:-4] for x in os.listdir("../../../../raw-results/parsing_coarse_withoutPOS2_plane/") if "estimateParseability" in x]
#print(parsingDone)

script = "estimateParseability_WithoutPOS2.py"
...
planeBasePath = "../../../../raw-results/parsing-withoutPOS/"
while failures < 300:
  ...
  if len(existing) > 0: #random.random() > ((1.0/(1+len(existing)))): # language == "Czech" or 
     print("Model for this model exists "+str(((1.0/(1+len(existing))))))
     failures += 1
     continue
  failures = 0

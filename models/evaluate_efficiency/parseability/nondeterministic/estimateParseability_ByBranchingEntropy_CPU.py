...
pos_u_embeddings = torch.nn.Embedding(num_embeddings = len(posUni)+3, embedding_dim = pos_embedding_size)



dropout = nn.Dropout(dropout_rate)

rnn = nn.LSTM(pos_embedding_size, rnn_dim, lstm_layers, bidirectional=True, batch_first=True) # word_embedding_size + (2*
...
headRep = nn.Linear(2*rnn_dim, bilinearSize)
depRep = nn.Linear(2*rnn_dim, bilinearSize)

headMLPOut = nn.Linear(bilinearSize, bilinearSize)
dependentMLPOut = nn.Linear(bilinearSize, bilinearSize)

labelMLP = nn.Linear(2*bilinearSize, labelMLPDimension)
labelDecoder = nn.Linear(labelMLPDimension, len(itos_pure_deps)+1)


U = nn.Parameter(torch.Tensor(bilinearSize,bilinearSize))

biasHead = nn.Parameter(torch.Tensor(1,bilinearSize,1))
...
def forward(current, computeAccuracy=False, doDropout=True):
       ...
       if True:
           pos_u_layer = pos_u_embeddings(Variable(torch.LongTensor(input_pos_u).transpose(0,1)))
           ...
           targetTensorVariable = Variable(torch.LongTensor(targetTensor))
           targetTensorLong = torch.LongTensor(targetTensor)
           ...
           labelTargetTensorVariable = Variable(torch.LongTensor(labelTargetTensor))
...
while True:
  ...
  if True:
         ...
         with open("../../../../raw-results/parsing-nondeterministic/performance-"+language+"_"+FILE_NAME+"_model_"+str(myID)+"_"+model+".txt", "w") as outFile:

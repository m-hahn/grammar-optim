...
library("brms")
dependency = "nmod"
dependencies = c("acl", "advmod", "aux", "lifted_case", "lifted_cop", "lifted_mark", "nmod", "nsubj", "obl", "xcomp") # "nsubj",  "advmod",
...
get_prior(acl ~ (1|p|Family) + (1|q|Language), family="bernoulli", data=data)

model3 = brm(acl ~ (1|p|Family) + (1|q|Language), family="bernoulli", data=data, iter=5000)
model3 = brm(acl ~ (1|p|Family) + (1|q|Language), family="bernoulli", data=data, iter=5000, prior=prior(normal(0,2), class=Intercept))


# Get the default priors
get_prior(cbind(acl, advmod, aux, lifted_case, lifted_cop, lifted_mark, nmod, nsubj, obl, xcomp) ~ (1|p|Family) + (1|q|Language), family="bernoulli", data=data)

model3 = brm(cbind(acl, advmod, aux, lifted_case, lifted_cop, lifted_mark, nmod, nsubj, obl, xcomp) ~ (1|p|Family) + (1|q|Language), family="bernoulli", data=data, iter=5000, prior=prior(normal(0,2), class=Intercept))
# advmod, nsubj, 
...
#ggsave(plot=plot, filename="figures/posterior-satisfied-universals-efficiency-large-prior.pdf")
#write.csv(u, file="~/CS_SCR/posteriors/posterior-10-efficiency-large-prior.csv")

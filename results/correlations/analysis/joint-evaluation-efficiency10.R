# For the \lambda=1.0 simulation


data = read.csv("../../grammars/manual_output_funchead_two_coarse_final/auto-summary-lstm.tsv", sep="\t")# %>% rename(Quality=AverageLength)
library(forcats)
library(dplyr)
library(tidyr)
library(ggplot2)
...
u = posterior_samples(model3) #cbind(posterior_samples(model3, "acl_Intercept"))
...
ggsave(plot=plot, filename="figures/posterior-satisfied-universals-first10-efficiency.pdf")


write.csv(u, file="~/CS_SCR/posteriors/posterior-10-first10-efficiency.csv")

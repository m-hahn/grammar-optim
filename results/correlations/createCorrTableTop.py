...
patterns.append(("verb","subject","nsubj"))
patterns.append(("verb","manner adverb","advmod"))
...
with open("results-prevalence-depl-best-top.tsv", "r") as inFile:
    depl = tableByFirst([x.split("\t") for x in inFile.read().strip().split("\n")])
with open("results-prevalence-two-09-best-top.tsv", "r") as inFile:
    efficiency = tableByFirst([x.split("\t") for x in inFile.read().strip().split("\n")])
with open("results-prevalence-langmod-best-top.tsv", "r") as inFile:
    surp = tableByFirst([x.split("\t") for x in inFile.read().strip().split("\n")])
with open("results-prevalence-parse-best-top.tsv", "r") as inFile:

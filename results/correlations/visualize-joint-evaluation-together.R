...
samplesPerType = data %>% group_by(Type) %>% summarise(SamplesNum = nrow(satisfied))

data = merge(data, samplesPerType, by=c("Type"))

data2 = data %>% group_by(Type, satisfiedCount) %>% summarise(posterior = NROW(satisfiedCount)) %>% mutate(posteriorProb = posterior/SamplesNum)

...
data = read.csv("CS_SCR/deps/manual_output_funchead_two_coarse_parser/auto-summary-lstm.tsv", sep="\t")# %>% rename(Quality=AverageLength)
...
sink("results-parser.tsv")
cat("")
sink()

cat(paste("dependency", "satisfiedFraction", "posteriorMean", "posteriorSD", "posteriorOpposite", sep="\t"), file="results-parser.tsv", append=TRUE, sep="\n")

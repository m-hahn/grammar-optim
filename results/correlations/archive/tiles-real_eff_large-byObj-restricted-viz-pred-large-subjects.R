...
#bestEff = best %>% group_by(Language) %>% summarise(DLM = min(DLM))
#best = merge(best, bestEff, by=c("Language", "DLM"))
library(dplyr)
library(tidyr)
library(ggplot2)
library(forcats)
data = data %>% mutate(Language = fct_recode(Language, "Old_Church_Slavonic" = "Old", "Ancient_Greek"= "Ancient")) %>% rename(CoarseDependency = Dependency)
#best = best %>% mutate(Language = fct_recode(Language, "Old_Church_Slavonic" = "Old", "Ancient_Greek"= "Ancient"))
data = data %>% mutate(FileName = as.character(FileName)) #, best %>% rename(FileName = Model), by=c("Language", "FileName"))
data = data
languages = read.csv("../languages/languages-iso_codes.tsv", sep=",")
languages = languages %>% mutate(Family = ifelse(grepl("Dravid", Family), "Dravidian", as.character(Family)))
languages = languages %>% mutate(Family = ifelse(grepl("Turk", Family), "Turkic", as.character(Family)))
...
#plot = ggplot(D %>% filter(Type == "DLM"), aes(x = Family, y = Language_Ordered)) + 

...
plot_orders_real = ggplot(D %>% filter(Type == "Real Languages"), aes(x = 1, y = LanguageNumeric, group=CoarseDependency)) + 
...
plot_orders_eff = ggplot(D %>% filter(Type == "Efficiency"), aes(x = 1, y = LanguageNumeric, group=CoarseDependency)) + 
  geom_point(aes(fill=DirB, colour = DirB, size =1), position = position_dodge(width=2.0)) +
#  scale_color_gradient() + #values=c("blue", "green")) +
  theme_bw() + theme(axis.text.x=element_blank(), #element_text(size=9, angle=0, vjust=0.3),
                     axis.text.y=element_blank(),axis.ticks=element_blank(),
  ...
  theme(axis.title=element_blank()) + 
  theme(legend.position="none") + labs(x=NULL)
DLang = unique(D %>% select(Language_Ordered, iso_Ordered, LanguageNumeric))
DFam = D %>% group_by(Family) %>% summarise(Start = min(LanguageNumeric), End = max(LanguageNumeric), Mean = mean(LanguageNumeric))
...
plot_langs = plot_langs +  theme_bw() + theme(axis.text.x=element_blank(), #element_text(size=9, angle=0, vjust=0.3),
                     ...
                     plot.title=element_text(size=11)) 
plot_langs = plot_langs + geom_text(aes(x=1.2 + 0.05, y=LanguageNumeric, label=iso_Ordered), hjust=1, size=3, colour="grey30")

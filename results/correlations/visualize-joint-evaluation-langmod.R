u = read.csv("/home/user/CS_SCR/posteriors/posterior-10-langmod.csv")
...
plot = ggplot(data = data2, aes(x=satisfiedCount, y=posterior)) + geom_bar(stat="identity") + theme_bw() + xlim(0,10.5) + ggtitle("Predictability") + xlab("Number of Predicted Correlations") + ylab("Number of Posterior Samples") + geom_text(aes(label=posteriorProb), vjust=0) + ylim(0, 1.1*nrow(data))

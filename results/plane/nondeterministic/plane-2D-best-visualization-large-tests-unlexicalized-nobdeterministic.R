...
library(tidyr)
library(dplyr)
dataS =  read.csv("../../../grammars/plane/controls/plane-fixed-withoutPOS-nondeterministic.tsv", sep="\t") %>% mutate(Model = as.character(Model))
...
dataP =  read.csv("../../../grammars/plane/controls/plane-parse-lexicalized-nondeterministic.tsv", sep="\t") %>% mutate(Model = as.character(Model))
...
sink("per-language-statistics-unlexicalized-nondeterministic.tex")
for(i in (1:51)) {

...
dataP =  read.csv("../../../grammars/plane/controls/plane-parse-nondeterministic.tsv", sep="\t")  %>% mutate(Model = as.character(Model), Type=as.character(Type)) %>% mutate(Type = ifelse(grepl("RANDOM", Type), "manual_output_funchead_RANDOM_nondet", Type)) %>% mutate(FullSurp=NULL) %>% mutate(Model = paste(Model, "nondet", sep="_"))
dataP2 = read.csv("../../../grammars/plane/plane-parse-redone.tsv", sep="\t")  %>% mutate(Model = as.character(Model), Type=as.character(Type)) %>% mutate(Type = ifelse(grepl("RANDOM", Type), "manual_output_funchead_RANDOM_det", Type)) %>% mutate(FullSurp=NULL) %>% mutate(Model = paste(Model, "det", sep="_"))
...
plot = ggplot(data %>% filter(grepl("RANDOM", Type)) %>% filter(abs(Surp_z) < 3), aes(x=-Pars_z, y=-Surp_z, color=Type, group=Type))

...
for(language in unique(data$Language)) {
    ...
    pars = data2$Pars
    cat(language,"\n")
    dataIso = read.csv(file=paste("../../grammars/pareto-curves/pareto-smooth/iso-pareto-full-", language, sep="")) %>% mutate(Language = language)
    ...
    dataIso = dataIso %>% group_by(Language, x) %>% summarise(y_sd = sd(y), y = mean(y))

    dataIso = dataIso %>% filter(-y < 1.0+max(-(subData %>% filter(Language == language))$Surp_z))
    dataIso_ = rbind(dataIso_, as.data.frame(dataIso))



    dataAlpha = read.csv(file=paste("../../grammars/pareto-curves/pareto-smooth/iso-pareto-alphas-", language, sep="")) %>% mutate(Language = language)
dataAlpha = dataAlpha %>% group_by(Language) %>% summarise(xhigh = quantile(x, 0.975), xlow = quantile(x, 0.025), x=mean(x))
    dataAlpha_ = rbind(dataAlpha_, as.data.frame(dataAlpha))
...
plot = plot + geom_density_2d(data=data %>% filter(Type == "manual_output_funchead_RANDOM") %>% filter(Surp_z < 3), aes(x=-Pars_z, y=-Surp_z, color=Type, group=Type), size=0.3, bins=5)
plot = plot + geom_path(data=subData, aes(x=-Pars_z, y=-Surp_z, group=1), size=1.5)
plot = plot + geom_line(data=dataIso_ %>% mutate(Type = "Pareto"), aes(x=-x, y=-y), size=1.5, color="orange")
plot = plot + geom_line(data=dataIso_ %>% mutate(Type = "Pareto"), aes(x=-x, y=-y-y_sd), size=0.5, color="orange")
plot = plot + geom_line(data=dataIso_ %>% mutate(Type = "Pareto"), aes(x=-x, y=-y+y_sd), size=0.5, color="orange")
#plot = plot + geom_ribbon(data=dataIso_ %>% mutate(Type = "Pareto"), aes(x=-x, ymin=-y-y_sd, ymax=-y+y_sd))
...
plot = plot + geom_label(data=dataAlpha_ %>% mutate(Pars_z=Inf, Surp_z=-Inf, Type = "Pareto", hjust=-0.5, vjust=1.0), aes(label=format(dataAlpha_$x, digits=2)), vjust=1.0,hjust="inward", color="black") #, hjust=hjust, vjust=vjust)))
plot = plot + geom_text(data=dataAlpha_ %>% mutate(Pars_z=Inf, Surp_z=-Inf, Type = "Pareto", hjust=-0.5, vjust=1.0), aes(label=paste("[", format(dataAlpha_$xlow, digits=2), ",", format(dataAlpha_$xhigh, digits=2), "]", sep="")), vjust=3.5,hjust="inward", size=3.0, color="black") #, hjust=hjust, vjust=vjust)))
...
ggsave(plot, file="pareto-plane-perLanguage-isoCurve-smooth-interval.pdf", width=12, height=12)

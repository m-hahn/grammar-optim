...
plot = ggplot(data, aes(x=(Pars-MeanPars)/SDPars, y=(Surp-MeanSurp)/SDSurp, color=Type, group=Type, alpha=0.9)) +geom_point()  + theme_bw() + geom_point(data=data %>% filter(grepl("ground", Type)), aes(x=(Pars-MeanPars)/SDPars, y=(Surp-MeanSurp)/SDSurp, color=Type, group=Type, alpha=1.0))  + geom_density_2d(data=data %>% filter(grepl("RANDOM", Type)), aes(alpha=1.0)) + geom_segment(data=dataGroundArrow, aes(x=Pars_z, y=Surp_z, xend=Pars_z_end, yend = Surp_z_end, color=Type, group=Type, alpha=1.0)) + geom_path(data=summarizedData %>% filter(Type %in% c("manual_output_funchead_two_coarse_parser_final", "manual_output_funchead_two_coarse_final", "manual_output_funchead_langmod_coarse_final")), aes(x=Pars, y=Surp, size=5, group=1, alpha=1.0)) + geom_point(data=summarizedData, aes(x=Pars, y=Surp, size=8, alpha=1.0)) + scale_x_continuous(name="Parsability") + scale_y_continuous(name="Surprisal") + theme(legend.position="none")

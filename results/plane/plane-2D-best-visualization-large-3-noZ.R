...
for(language in unique(subData$Language)) {
   ...
   pred = (u %>% filter(Type == "manual_output_funchead_two_coarse_lambda09_best_large"))$Surp[1]
   ...
   pred = (u %>% filter(Type == "manual_output_funchead_two_coarse_parser_best_balanced"))$Surp[1]
   pars = min(u$Pars)
...
plot = ggplot(data %>% filter(Type %in% c("manual_output_funchead_RANDOM", "manual_output_funchead_ground_coarse_final")) %>% filter(Surp_z < 3), aes(x=-Pars, y=-Surp, color=Type, group=Type))
...
plot = plot + theme_bw()
plot = plot + scale_x_continuous(name="Negative Ambiguity") + scale_y_continuous(name="Predictability")

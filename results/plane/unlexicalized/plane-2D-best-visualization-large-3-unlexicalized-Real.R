...
dataCompGround = dataComp %>% filter(Type == "REAL_REAL") %>% select(Language, Surp, Pars) %>% rename(SurpGround = Surp) %>% rename(ParsGround = Pars) %>% mutate(EffGround = ParsGround + 0.9*SurpGround) %>% group_by(Language)
...
dataReal = data %>% filter(grepl("REAL_REAL", Type)) #%>% group_by(Language) %>% summarise(Surp = mean(Surp), Pars = mean(Pars))
...
plot = ggplot(data %>% filter(Type %in% c("manual_output_funchead_RANDOM", "REAL_REAL")) %>% filter(Surp_z < 3), aes(x=-Pars_z, y=-Surp_z, color=Type, group=Type))
plot = plot + geom_point()
plot = plot + geom_path(data=subData, aes(x=-Pars_z, y=-Surp_z, group=1), size=1.5)
plot = plot + geom_point(data=data %>% filter(Type == "REAL_REAL"), size=2)
...
ggsave(plot, file="pareto-plane-perLanguage-lexicalized-real.pdf", width=12, height=12)

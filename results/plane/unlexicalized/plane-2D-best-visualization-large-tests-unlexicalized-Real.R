...
dataReal = data %>% filter(Type == "REAL_REAL") %>% select(Language, Surp, Pars) %>% rename(SurpReal = Surp) %>% rename(ParsReal = Pars) %>% mutate(EffReal = ParsReal + 0.9*SurpReal) %>% group_by(Language)
data = merge(dataBaseline, dataReal, by=c("Language"))

data$Eff = data$Pars + 0.9*data$Surp

u = data %>% group_by(Language) %>% summarise(BetterSurp = sum(Surp > SurpReal, na.rm=TRUE), WorseSurp = sum(Surp <= SurpReal, na.rm=TRUE), TotalSurp = BetterSurp+WorseSurp, BetterFracSurp = BetterSurp/TotalSurp, BetterPars = sum(Pars > ParsReal, na.rm=TRUE), WorsePars = sum(Pars <= ParsReal, na.rm=TRUE), TotalPars = BetterPars+WorsePars, BetterFracPars = BetterPars/TotalPars, BetterEff = sum(Eff > EffReal, na.rm=TRUE), WorseEff = sum(Eff <= EffReal, na.rm=TRUE), TotalEff = BetterEff+WorseEff, BetterFracEff = BetterEff/TotalEff )
...
sink("per-language-statistics-unlexicalized-real.tex")
for(i in (1:51)) {
	...
	language = u$Language[[i]]
        ...
        testSurp_t = t.test(v$Surp, mu=mean(v$SurpReal), alternative="greater")
	...
	language = u$Language[[i]]
        v = data %>% filter(Language == language)
        if(is.na(mean(v$EffReal))) {
        	pValuesEff_t = c(pValuesEff_t, 0.5)
        } else {
        	pValuesEff_t = c(pValuesEff_t, t.test(v$Eff, mu=mean(v$EffReal), alternative="greater")$p.value)
        }
	if(u$TotalPars[[i]] == 0) {
	...
	language = u$Language[[i]]
        v = data %>% filter(Language == language)
        if(is.na(mean(v$ParsReal))) {
                testPars_t = t.test(c(-1,1), mu=0, alternative="greater")
        	pValuesPars_t = c(pValuesPars_t, 0.5)
        } else {
                testPars_t = t.test(v$Pars, mu=mean(v$ParsReal), alternative="greater")

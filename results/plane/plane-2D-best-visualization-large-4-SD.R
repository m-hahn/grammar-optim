...
subData2 = subData %>% mutate(TypeN=Type) %>% group_by(Step) %>% summarise(Surp = mean(Surp_z), Pars = mean(Pars_z), Surp_SD = sd(Surp_z), Pars_SD = sd(Pars_z))
...
plot = plot + geom_path(data=subData2, aes(x=-Pars+Pars_SD, y=-Surp+Surp_SD, group=1), color="gray", size=2, linetype="dashed")
plot = plot + geom_path(data=subData2, aes(x=-Pars-Pars_SD, y=-Surp-Surp_SD, group=1), color="gray", size=2, linetype="dashed")

...
data = merge(dataS, dataP, by=c("Language", "Model", "Type"), all.x=TRUE, all.y=TRUE) %>% filter(Language %in% c("English", "Japanese"))

# Transferred manually from
# ~/scr/deps//language_modeling_coarse_plane_fixed_counter/
# ../../../../raw-results/parsing-upos2-counter/
data2 = data.frame(Language = c("English", "Japanese"), Model = c("3679914", "7520227"), Type = c("xground_other", "xground_other"), Surprisal = c(6.550901074296865, 6.273405530411384), Pars = c(0.7777210038622745, 0.7052601910851886))
...
plot = plot + geom_point(data=data %>% filter(Type %in% c("xground_other")) %>% filter(Surprisal_z < 3), shape=5, size=1.5, stroke=2)
...
ggsave(plot, file="pareto-plane-other-language.pdf", width=8, height=4)

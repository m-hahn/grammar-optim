...
dataCompGround = dataComp %>% filter(Type == "manual_output_funchead_ground_coarse_pureUD_final") %>% select(Language, Surp, Pars) %>% rename(SurpGround = Surp) %>% rename(ParsGround = Pars) %>% mutate(EffGround = ParsGround + 0.9*SurpGround) %>% group_by(Language)
...
dataGround = data %>% filter(grepl("ground_coarse_pureUD", Type)) #%>% group_by(Language) %>% summarise(Surp = mean(Surp), Pars = mean(Pars))
...
plot = ggplot(data %>% filter(Type %in% c("manual_output_funchead_RANDOM_pureUD", "manual_output_funchead_ground_coarse_pureUD_final")) %>% filter(Surp_z < 3), aes(x=-Pars_z, y=-Surp_z, color=Type, group=Type))
plot = plot + geom_point()
#plot = plot + geom_path(data=subData, aes(x=-Pars_z, y=-Surp_z, group=1), size=1.5)
plot = plot + geom_point(data=data %>% filter(Type == "manual_output_funchead_ground_coarse_pureUD_final"), size=2)
...
ggsave(plot, file="pareto-plane-perLanguage-pureUD-mle.pdf", width=12, height=12)

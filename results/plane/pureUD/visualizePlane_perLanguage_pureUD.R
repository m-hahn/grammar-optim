...
dataCompGround = dataComp %>% filter(Type == "manual_output_funchead_ground_coarse_pureUD_final") %>% select(Language, Surprisal, Pars) %>% rename(SurprisalGround = Surprisal) %>% rename(ParsGround = Pars) %>% mutate(EffGround = ParsGround + 0.9*SurprisalGround) %>% group_by(Language)
...
dataGround = data %>% filter(grepl("ground_coarse_pureUD", Type)) #%>% group_by(Language) %>% summarise(Surprisal = mean(Surprisal), Pars = mean(Pars))
...
plot = ggplot(data %>% filter(Type %in% c("manual_output_funchead_RANDOM_pureUD")) %>% filter(Surprisal_z < 3), aes(x=-Pars_z, y=-Surprisal_z, color=Type, group=Type))
plot = plot + geom_point()
#plot = plot + geom_path(data=subData, aes(x=-Pars_z, y=-Surprisal_z, group=1), size=1.5)
plot = plot + geom_point(data=data %>% filter(Type == "manual_output_funchead_ground_coarse_pureUD_final"), shape=4, size=1.5, stroke=2)

...
ggsave(plot, file="pareto-plane-iso-best-balanced-legend-viz-10_pareto.pdf")


# TODO Czech is omitted?!


subData2 = subData %>% mutate(TypeN=Type) %>% group_by(Step) %>% summarise(Surp = mean(Surp_z), Pars = mean(Pars_z))

plot = ggplot(dataPlot)  
plot = plot + theme_bw() 
#plot = plot + geom_segment(aes(x=-Pars_z, y=-Surp_z, xend=-Pars_z_end, yend = -Surp_z_end, color=TypeN, group=TypeN), size=0.6)
plot = plot + geom_density_2d(data=dataPlot %>% filter(grepl("Baseline", TypeN)), aes(x=-Pars, y=-Surp, color=TypeN, group=TypeN), size=0.3, bins=5)
plot = plot + geom_path(data=subData2, aes(x=-Pars, y=-Surp, group=1), color="gray", size=5)
#plot = plot + geom_point(data=dataPlot, aes(x=-ParsMean, y=-SurpMean, color=TypeN, group=TypeN), size=6)
plot = plot + scale_x_continuous(name="Parseability") + scale_y_continuous(name="Predictability")
...
ggsave(plot, file="pareto-plane-iso-best-balanced-legend-viz-10-fontsize_pareto.pdf", width=6.6, height=6.6)

...
dataGroundArrow = dataGroundArrow %>% mutate(Pars_z_end = Pars_z + 0.5 * Pars_z_dir, Surp_z_end = Surp_z + 0.5 * Surp_z_dir)
...
#plot = plot + geom_segment(data=dataGroundArrow, aes(x=-Pars_z, y=-Surp_z, xend=-Pars_z_end, yend = -Surp_z_end, color=Type, group=Type), size=2)
...
#ggsave(plot, file="pareto-plane-perLanguage-arrows.pdf", width=12, height=12)
...
data_pareto_ = data_pareto %>% group_by(Language) %>% summarise(bestLambdas_mean = mean(bestLambdas), bestLambdas_sd = sd(bestLambdas))
data_pareto_ = merge(data %>% filter(Type == "manual_output_funchead_ground_coarse_final"), data_pareto_, by=c("Language"))
data_pareto_line1 = data_pareto_ %>% mutate(Pars_z = ((Pars + 10)-MeanPars)/SDPars, Surp_z = ((Surp + 10*bestLambdas_mean)-MeanSurp)/SDSurp)
data_pareto_line2 = data_pareto_ %>% mutate(Pars_z = ((Pars - 10)-MeanPars)/SDPars, Surp_z = ((Surp - 10*bestLambdas_mean)-MeanSurp)/SDSurp)
...
plot = plot + geom_path(data=subData, aes(x=-Pars_z, y=-Surp_z, group=1), size=1.5)
plot = plot + geom_point(data=data %>% filter(Type == "manual_output_funchead_ground_coarse_final"), size=3)
#plot = plot + geom_line(data=data_pareto_line, aes(x=-Pars_z, y=-Surp_z, group=Language))
plot = plot + geom_abline(data=data_pareto_, aes(intercept=  -Surp_z - 1/((bestLambdas_mean+1e-10)/SDSurp) * Pars_z , slope=-1/((bestLambdas_mean+1e-10)/SDSurp), group=Language))
plot = plot + geom_segment(data=dataGroundArrow, aes(x=-Pars_z, y=-Surp_z, xend=-Pars_z_end, yend = -Surp_z_end, color=Type, group=Type), size=2)
plot = plot + facet_wrap(~Language, scales="free")
plot = plot + theme_bw()
plot = plot + scale_x_continuous(name="Parseability") + scale_y_continuous(name="Predictability")
...
data_baselines = data_baselines %>% mutate(Eff = Pars + bestLambdas_mean * Surp, EffGround = ParsGround + bestLambdas_mean * SurpGround)
